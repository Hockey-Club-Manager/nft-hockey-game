...
pub trait FungibleTokenCore {
    ...
    /// Both accounts must be registered with the contracts for transfer to succeed. (See [NEP-145](https://github.com/near/NEPs/discussions/145))
    ...
    /// calls `ft_on_transfer` method on `receiver_id` contracts and attaches a callback to resolve this transfer.
    /// `ft_on_transfer` method must return the amount of tokens unused by the receiver contracts, the remaining tokens
    /// must be refunded to the `predecessor_account_id` at the resolve transfer callback.
    ///
    /// Token contracts must pass all the remaining unused gas to the `ft_on_transfer` call.
    ///
    /// Malicious or invalid behavior by the receiver's contracts:
    /// - If the receiver contracts promise fails or returns invalid value, the full transfer amount must be refunded.
    /// - If the receiver contracts overspent the tokens, and the `receiver_id` balance is lower than the required refund
    ...
    ///
    /// Both accounts must be registered with the contracts for transfer to succeed. (See #145)
    ...
    ///
    /// Arguments:
    /// - `receiver_id` - the account ID of the receiver contracts. This contracts will be called.
    /// - `amount` - the amount of tokens to transfer. Must be a positive number in a decimal string representation.
    /// - `msg` - a string message that will be passed to `ft_on_transfer` contracts call.
    /// - `memo` - an optional string field in a free form to associate a memo with this transfer.
    ///
...
trait FungibleTokenReceiver {
    /// Called by fungible token contracts after `ft_transfer_call` was initiated by
    /// `sender_id` of the given `amount` with the transfer message given in `msg` field.
    /// The `amount` of tokens were already transferred to this contracts account and ready to be used.
    ///
    /// The method must return the amount of tokens that are *not* used/accepted by this contracts from the transferred
    /// amount. Examples:
    /// - The transferred amount was `500`, the contracts completely takes it and must return `0`.
